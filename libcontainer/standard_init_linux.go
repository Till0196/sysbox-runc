...
func (l *linuxStandardInit) Init() error {
	...
	// necessary because the execve target could use internal sysbox-runc fds as
	// the execve path, potentially giving access to binary files from the host
	// (which can then be opened by container processes, leading to container
	// escapes). Note that because this operation will close any open file
	// descriptors that are referenced by (*os.File) handles from underneath the
